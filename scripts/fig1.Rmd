...
# BiocManager::install(c('msa','ggtree', 'ggtreeExtra'))
# install.packages(c('ggpubr', 'patchwork', 'vegan'))
library(ggpubr)
library(patchwork)
...
seed_culture_data <- read_tsv("./data/sequence_data/geomicro_cultures_w_culture_label.tsv") |>
  ...
  dplyr::rename(`LE19-196\\.1\n (seed)` = `LE19-196.1`,
         `LE17-020\n (seed)` = `LE17-020`) |>
  pivot_longer(cols = c(`LE19-196\\.1\n (seed)`, `LE17-020\n (seed)`), names_to = "sample", values_to = "rel_abund") |>
  filter(rel_abund > 0) |>
...
nmds_bray <- nmds_coords %>% 
  ...
  geom_text(aes(label = sample, color = sample),show.legend = F,family = 'arial', size = 8/.pt) +
  scale_color_manual(values = c("gray40","gray40", "cyan3","firebrick3","chartreuse3","darkgoldenrod3")) +
  labs(color = NULL) +
  xlim(-1.5,1.5) +
  ylim(-0.01,0.01) +
  theme_pubr(base_family = 'arial',margin = F, border = F, base_size = 8)
...
final_tree <- gheatmap(r, abund_matrix, offset =0.3, width = 2, color = "gray70",
         colnames = F, legend_title = "Relative Abundance \n(within column)",low = "white", high = "chartreuse3",family = 'arial', font.size = 8) +
  xlab("Branch Length") +
  theme_pubr(base_family = 'arial', margin = F, border = F, base_size = 8) +
  theme(axis.text.y = element_blank(),
        ...
        plot.tag = element_text(size = 9.5, family = 'arial')) +
    scale_x_ggtree() + 
    scale_y_continuous(expand=c(0,1))

combo_plot <- (final_tree / nmds_bray) + plot_annotation(tag_prefix = '[', tag_suffix = ']',tag_levels = "A") + plot_layout(widths = c(5,1))

fig_1 <- ggarrange(ncol = 2, labels = c("[A]","[B]"),final_tree, nmds_bray, widths = c(2,1),
                   font.label = list(size = 9.5, color = "black"))

fig_1B <- nmds_bray + plot_annotation(tag_prefix = '[', tag_suffix = ']',tag_levels = "A") &
  theme(plot.tag = element_text(face = 'bold', size = 9.5, family = 'arial'))
showtext_opts(dpi = 600)
ggsave('./figures/figure_pieces/figure1.png',combo_plot,width = 180, height = 152, units = "mm", dpi = 600)
ggsave('./figures/figure_pieces/figure1.svg',device = svglite::svglite, combo_plot,width = 180, height = 152, units = "mm", dpi = 600)
ggsave('./figures/figure_pieces/figure1A.png',fig_1A,width = 180, height = 100, units = "mm", dpi = 600)
ggsave('./figures/figure_pieces/figure1A.svg',device = svglite::svglite,final_tree,width = 180, height = 100, units = "mm", dpi = 600)
ggsave('./figures/figure_pieces/figure1B.png',fig_1B,width = 180, height = 80, units = "mm", dpi = 600)
ggsave('./figures/figure_pieces/figure1B.svg',device = svglite::svglite,fig_1B,width = 180, height = 100, units = "mm", dpi = 600)
