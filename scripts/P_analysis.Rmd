...
ggsave('./figures/Finalized Figs for Publication/figure3_final.png',PC_plot_P,width = 180, height = 225, units = "mm", dpi = 600)
...
Monod_patchwork <- (gr_PC_plot | fig4_Nmonod) + plot_annotation(tag_levels = "A") + plot_layout(guides = 'collect')

ggsave('./figures/Finalized Figs for Publication/figure4_final.png',Monod_patchwork,width = 180, height = 200, units = "mm", dpi = 600)
...
grs_PC <- inner_join(results_P_PC, std_error_P, by = c("Well","Plate","State","P_ug_L","Culture")) |>
  ...
  mutate(growthrate_corrected = case_when(r2 <= max(r2[Culture == "Media_CTRL"]) ~ 0, 
...
gr_PC_plot <- ggplot(data = filter(grs_PC,growthrate_corrected > 0), aes(x = P_ug_L, y = growthrate)) +
   geom_point(color = "orange",
              size = 2) +
   ylab("μ (1/day)") +
   xlab("Phosphorus (μg/L)") +
