...
full_data <-  inner_join(data_all, map_data,
  ...
  dplyr::mutate(
    evap_correction = (1 - read_timeDays * 0.002761443318),
         PC_150_true = abs(PC_150 - mean(PC_150[Culture == "Media_CTRL"])) * evap_correction,
...
grs_PC <- inner_join(results_P_PC, std_error_P, by = c("Well","Plate","State","P_ug_L","Culture")) %>%
  ...
  mutate(growthrate_corrected = ifelse(p.value < 0.05 & r2 > 0.9, growthrate, 0)) %>%
  #filter((!growthrate_corrected == 0 | N_ug_L < 100)) %>%
  dplyr::select(Well, Plate, State,
         P_ug_L,lag, growthrate_corrected, Treatment_name,
         growthrate, Culture,
         r2, std_error, p.value)

gr_chlA_plot_auto <- ggplot(data = filter(grs_chlA, Culture != "LEP"), 
                       aes(x = P_ug_L, y = growthrate)) +
   geom_point(color = "darkgreen",
              size = 2) +
   ylab("μ (1/day)") +
   xlab("Phosphorus (μg/L)") +
   facet_grid(~factor(Culture, 
                     levels = c("CS", "SA", "MM", "LE"), 
                     labels = c("C. sorokiniana", "S. acuminatus", 
                                "M. minutum", "Mychonastes sp."))) +
   stat_smooth(color = "black", 
               size = 0.7,
               method = "nls",
               se = FALSE,
               method.args = list(
                 formula = y~mumax*x/(x + ks),
                 start = list(mumax = .75, ks = 5)
               )) +
  geom_errorbar(mapping = aes(ymin = growthrate_corrected - std_error,
                              ymax = growthrate_corrected + std_error, width = 20)) +
  ylim(0,1) +
