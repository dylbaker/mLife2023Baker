...
sig_results <- rbind(gnls_N, gnls_P) |> 
  filter(str_detect(psig.adj, "\\*"), !str_detect(term, "Intercept"))
...
summary_stats <- rbind(pccax,pccx,nccax,nccx) |>
  dplyr::rename("Contrast" = Form,
                ...
                "(SE)" = SE) |>
  mutate(Parameter = case_when(str_detect(Contrast, "ks") ~ "*K*<sub>s</sub>",
                               T ~ "*μ*<sub>max</sub>")) |>
...
contrasts_barplot <- ggplot(data = summary_stats, aes(x = State, y = mean_abs_diff)) +
  ...
  theme_pubr(base_size = 8, base_family = 'arial',border = T, margin = F) +
  theme(legend.position = "right",
        strip.text = element_markdown()
        )

png(file = "./figures/Finalized Figs for Publication/FigureS2_final.png", 
    type = "cairo",
    units = "mm",
    res = 600, width = 160, height = 100)
