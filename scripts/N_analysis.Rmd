...
plot_data <- rbind(all_facets, sample_facets)|>
  ...
  dplyr::mutate(Culture = factor(Culture, levels = c("MUT_7806","WT_7806","PCC_9701", "NIES_843","Media_CTRL")))

PC_plot_N <- ggplot(data = plot_data, aes(x = read_timeDays, y = log10(PC_100_true), color = Culture)) +
   geom_point(size = 1.5, shape = 21) +
   scale_color_manual(values = c("WT_7806" =  "firebrick3",
   ...
   xlab("Time (days)") +
   xlim(0, 17) +
   coord_cartesian(ylim = c(0,3.4), expand = T) +
...
fig_S1 <- ggarrange(ncol = 2, labels = c('[A]','[B]'), gr_PC_plot, gr_chlA_plot,font.label = list(size = 9.5))

ggsave('./figures/Finalized Figs for Publication/FigureS1_final.png',fig_S1,width = 160, height = 160, units = "mm", dpi = 600)
